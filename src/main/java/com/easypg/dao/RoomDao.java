...
public interface RoomDao extends JpaRepository<Room, Long> {
  ...
  @Query("SELECT DISTINCT r FROM Room r LEFT JOIN FETCH r.facilities f WHERE r.hidden = false AND r.isAvailable =true AND (f.isDeleted = false OR f IS NULL) AND r.currentOccupancy <= r.maxOccupancy")
